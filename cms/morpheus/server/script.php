...
if ($error) {
	...
	// warum dieser unsinn???? was für ein prob gab es ?
	$new_name = eliminiere($_FILES['Filedata']['name']);
#	$new_name = substr($new_name, 0, -3).".jpg";
	
	move_uploaded_file($_FILES['Filedata']['tmp_name'], "../../images/kunden/high/".$new_name);
	chmod("../../images/kunden/high/".$new_name, 0777);	
	konvert_img ("../../images/kunden/high/".$new_name, $new_name, 30, "../../images/kunden/thumb/");
	
	$query  = "SELECT * FROM p_images WHERE bereich=".$_POST["bereich"]." AND kid=".$_POST["kid"];
	$res = safe_query($query);
	$x = mysql_num_rows($res);
	$x++;

	$query  = "INSERT p_images set bereich=".$_POST["bereich"].", kid=".$_POST["kid"].", image='".$new_name."', reihenfolge=$x";
	$res = safe_query($query);
}
