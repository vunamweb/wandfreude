...
class ModelCheckoutOrder extends Model {
	//dream robot
	public function createTableSaveToken()
    {
        $sql = "CREATE TABLE " . DB_PREFIX . "token_dream_robot(
	...
	public function createTableSaveTokenPrint()
    {
        $sql = "CREATE TABLE " . DB_PREFIX . "token_print(
                token text NOT NULL,
                create_date DATETIME NOT NULL
                )";
    ...
    public function createTableSaveOrderDreamRobot()
    {
        $sql = "CREATE TABLE " . DB_PREFIX . "order_id_dream_robot(
                order_id int NOT NULL,
                order_id_dream_robot VARCHAR(250) NOT NULL
                )";

        $this->db->query($sql);
    }
    
    public function getOrderInDreamrobot($order_id) {
        $sql = "select * from ".DB_PREFIX."order_id_dream_robot where order_id=".$order_id."";
        $row = $this->db->query($sql);
        
        //echo $row->num_rows; die(); 
        
        if($row->num_rows) {
            $row = $row->rows;
            //print_r($row); die();
            
            return $row[0]['order_id_dream_robot'];
        }
        
        return null;
    }
    
    public function insertOrderDreamrobot($order_id, $order_id_dreamrobot) {
        $sql = "INSERT INTO " . DB_PREFIX . "order_id_dream_robot(order_id, order_id_dream_robot)values(".$order_id.", '".$order_id_dreamrobot."')";
        $this->db->query($sql);
    }
    
    public function expiredToken() {
        $sql = "select * from ".DB_PREFIX."token_dream_robot";
        $row = $this->db->query($sql);
        ...
        if($row->num_rows) {
           ...
           if(strtotime($current_date) - strtotime($create_date) <= 3600 )
             return false;
           else 
             return true;  
        }
	...
	public function expiredTokenPrint() {
        $sql = "select * from ".DB_PREFIX."token_print";
        $row = $this->db->query($sql);
        ...
        if($row->num_rows) {
           ...
           if(strtotime($current_date) - strtotime($create_date) <= 86400 )
             return false;
           else 
             return true;  
        }
    ...
    public function insertToken($token, $create_date) {
        $sql = "delete from " . DB_PREFIX . "token_dream_robot ";
        $this->db->query($sql);
        
        $sql = "INSERT INTO " . DB_PREFIX . "token_dream_robot(token, create_date)values('".$token."', '".$create_date."')";
        $this->db->query($sql);
	}
	
	public function insertTokenPrint($token, $create_date) {
        $sql = "delete from " . DB_PREFIX . "token_print";
        $this->db->query($sql);
        
        $sql = "INSERT INTO " . DB_PREFIX . "token_print(token, create_date)values('".$token."', '".$create_date."')";
        $this->db->query($sql);
    }
    
    public function getToken() {
        $sql = "select token from ".DB_PREFIX."token_dream_robot";
	...
	public function getTokenPrint() {
        $sql = "select token from ".DB_PREFIX."token_print";
	...
	public function getProductImage($product_id)
    {
        ...
        return HTTPS_SERVER . $rows[0]['image'];
	}
	
	public function getProductConfigurationFormTotal($data) {
		$response = $data;

        for($i = 0; $i < count($response['products']); $i++) {
            $product_id = $response['products'][$i]['product_id'];

            if($this->getTypeProduct($product_id) == 2)
              unset($response['products'][$i]);
        }

        return $response;
	}
	
	function sendMailSMTPWithAttach($to, $subject, $message)
   {
		$files1 = str_replace("index.php", "", $_SERVER['SCRIPT_FILENAME']) . "pdf/Auftragsbestaetigung.pdf";
		$files2 = str_replace("index.php", "", $_SERVER['SCRIPT_FILENAME']) . "pdf/AGB-wandfreude-GmbH.pdf";
		$files3 = str_replace("index.php", "", $_SERVER['SCRIPT_FILENAME']) . "pdf/Widerrufsrecht-wandfreude-GmbH.pdf";
		...
		$mail->Host = "w01b91dc.kasserver.com"; // sets GMAIL as the SMTP server
		$mail->Port = 465; // set the SMTP port for the GMAIL server
		$mail->Username = "info@wandfreude.de"; // GMAIL username
		$mail->Password = "mail5wandfreudegmbh";
		$mail->AddAddress($to);
		$mail->Subject = $subject;
		$mail->FromName = $this->config->get('config_email');
		$mail->From = "info@wandfreude.de";
		$mail->IsHTML(true);
		$mail->Body = utf8_decode(str_replace("€", " &euro;", $message));
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
        ...
        mail('vukynamkhtn@gmail.com', "test", "hi hi", $headers); die();*/
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['text_greeting'] = sprintf($language->get('text_new_greeting'), $order_info['payment_firstname'] . ' ' . $order_info['payment_lastname']);
				...
				foreach ($order_product_query->rows as $product) {
                    ...
                    $order_url_img_query = $this->db->query("SELECT image FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product['product_id'] . "'");
	                $order_url_img_query = $order_url_img_query->rows;

                    $order_another_information_query = $this->db->query("SELECT another_information FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product['product_id'] . "'");
	                $order_another_information_query = $order_another_information_query->rows;


					$data['products'][] = array(
						'image'     => $order_url_img_query[0]['image'],
                        'another_information'     => ($order_another_information_query[0]['another_information'] != '') ? json_decode($order_another_information_query[0]['another_information']) : '',
						'name'     => $product['name'],
...
$text .= isset($data['stripecw_payment_information']) ? Customweb_Core_Util_Html::toText($data['stripecw_payment_information']) : '';
                ...
                //mail("vukynamkhtn@gmail.com", $subject, "this is mail test hhh");

                //create pdf
                $dompdf = new Dompdf();
                $dompdf->loadHtml($this->load->view('mail/order_pdf', $data));
                //$dompdf->loadHtml("dffdfdf");
                $dompdf->setPaper('A4', 'Horizontal');
                $dompdf->render();
                $pdf = $dompdf->output();
                $file_location = "./pdf/Auftragsbestaetigung.pdf";
                file_put_contents($file_location,$pdf);
                //end
                //send email with attach pdf

                 $files = "./pdf/Auftragsbestaetigung.pdf";
                 $filename= "Auftragsbestaetigung.pdf";
                  $fp = fopen($files,"rb");
                  $data_ = fread($fp,filesize($files));
                 fclose($fp);
                 $data_ = chunk_split(base64_encode($data_));

				 //end
				 
				 
                //send email with attach pdf2
                $files2 = "./pdf/AGB-wandfreude-GmbH.pdf";
                $files3 = "./pdf/Widerrufsrecht-wandfreude-GmbH.pdf";
                $filename2= "AGB-wandfreude-GmbH.pdf";
                $filename3 = "Widerrufsrecht-wandfreude-GmbH.pdf";

                $fp = fopen($files2,"rb");
                $data_2 = fread($fp,filesize($files2));
                fclose($fp);
                $data_2 = chunk_split(base64_encode($data_2));

                $fp = fopen($files3,"rb");
                $data_3 = fread($fp,filesize($files3));
                fclose($fp);
                $data_3 = chunk_split(base64_encode($data_3));
				 //end
				 
				$email_body = $this->load->view('mail/order', $data);
				$email_body_1 = $this->load->view('mail/order', $this->getProductConfigurationFormTotal($data));
				...
				$message_configuration = "This is a multi-part message in MIME format.\n\n" . "--{$mime_boundary}\n" . "Content-Type: text/html; charset=\"UTF-8\"\n" . "Content-Transfer-Encoding: 7bit\n\n" . $email_body_1 . "\n\n";
                $message_configuration .= "--{$mime_boundary}\n";
                ...
                $message2 = "Content-Type: application/octet-stream; name=\"".basename($filename2)."\"\n" .
                        "Content-Description: ".basename($filename2)."\n" .
                        "Content-Disposition: attachment;\n" . " filename=\"".basename($filename2)."\"; size=".filesize($files2).";\n" .
                        "Content-Transfer-Encoding: base64\n\n" . $data_2 . "\n\n";

                $message2 .= "--{$mime_boundary}\n";

                $message2 .= "Content-Type: application/octet-stream; name=\"".basename($filename3)."\"\n" .
                        "Content-Description: ".basename($filename3)."\n" .
                        "Content-Disposition: attachment;\n" . " filename=\"".basename($filename3)."\"; size=".filesize($files3).";\n" .
                        "Content-Transfer-Encoding: base64\n\n" . $data_3 . "\n\n";
				...
				//mail("vukynamkhtn@gmail.com", "test", json_encode($data['products']));
				//die();
                //mail("vukynamkhtn@gmail.com", $subject, "this is mail test nambu");
				//mail($order_info['email'], $subject, $message, $headers);

				$this->sendMailSMTPWithAttach($order_info['email'], $subject, $email_body);
				$this->sendMailSMTPWithAttach("info@wandfreude.de", $subject, $message);
				
				//mail("info@wandfreude.de", $subject, $message_configuration, $headers);
