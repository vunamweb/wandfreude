...
class ControllerProductProduct extends Controller
{
    ...
    {
        $name = 'duschrueckwaende-kuechenrueckwaende-individuell-gestalten';

        if(strpos($_SERVER['REQUEST_URI'], '?product_id=')) {
            /*echo 'nambu';
            die();*/
            ...
            $alias = $this->model_catalog_product->getSEOInformation($product_id);
            $keyword = $alias['keyword'];

            if($keyword != '') {
                $url = $url = constant('HTTPS_SERVER') . 'angebote/' . $keyword;
        ...
        if ($_SERVER['REQUEST_URI'] == '/konfigurator/1' || $_SERVER['REQUEST_URI'] == '/konfigurator/'.$name.'/1') {
            $this->response->redirect('/duschrueckwaende-kuechenrueckwaende-individuell-gestalten/1');
        } else if ($_SERVER['REQUEST_URI'] == '/konfigurator/2' || $_SERVER['REQUEST_URI'] == '/konfigurator/'.$name.'/2') {
            $this->response->redirect('/duschrueckwaende-kuechenrueckwaende-individuell-gestalten/2');
        } else if ($_SERVER['REQUEST_URI'] == '/konfigurator/3' || $_SERVER['REQUEST_URI'] == '/konfigurator/'.$name.'/3') {
            $this->response->redirect('/duschrueckwaende-kuechenrueckwaende-individuell-gestalten/3');
        }  
        ...
        } else if($_GET['category'] == $name) {
            $url = $_GET['product_id'];

            $numberPlate = $_GET['number_plate'];

            $type= $_GET['type'];

            if($type == 0)
              $this->response->redirect('/product/'.$url.'/'.$numberPlate.'');
            else
              $this->response->redirect('/category/'.$url.'/'.$numberPlate.'');   
              
        } else if(strpos($_SERVER['REQUEST_URI'], 'category/') || strpos($_SERVER['REQUEST_URI'], 'product/')) {
            //print_r($_GET); die();
            $this->load->model('catalog/category');
            $this->load->model('catalog/product');

            $category_info = $this->model_catalog_category->getCategory($_GET['category']);
            $product_info = $this->model_catalog_product->getProduct($_GET['product_id']);
            $numberPlate = $_GET['number_plate'];
            
            $nameCategory = $this->convertNameUrl($category_info['name']);
            $nameProduct = $this->convertNameUrl($product_info['name']);

            $url = constant('HTTPS_SERVER') . 'duschrueckwand/'. $nameCategory .'/' . $nameProduct . '/' . $_GET['product_id'] . '/' . $_GET['category'] . '/' . $numberPlate.'';

            Header( "HTTP/1.1 301 Moved Permanently" );
            Header( "Location: ".$url."" );
            
            //$this->response->redirect('/duschrueckwand/'. $nameCategory .'/' . $nameProduct . '/' . $_GET['product_id'] . '/' . $_GET['category'] . '/' . $numberPlate.'');  
        } else if(strpos($_SERVER['REQUEST_URI'], '?product_id=')) {
            /*echo 'dd'; die();
            $product_id = $_GET['product_id'];
        ...
        if($product_info['type_product'] == 1 && $_GET['type_product'] == '') {
            $product_id = $_GET['product_id'];
            ...
            $url = constant('HTTPS_SERVER') . 'duschrueckwand/'. $nameCategory .'/' . $nameProduct . '/' . $_GET['product_id'] . '/' . $category_id . '/2';
        ...
        if ($product_info && ($product_info['type_product'] == 0 || isset($_GET['type_product']))) {
            ...
            $data['text_configuration'] = $product_info['text_configuration'];
            ...
            if (isset($_GET[type_product]) && $_GET[type_product] != '') {
                $left_content = $this->model_catalog_product->getLeftContent(21);
                $data['left_content'] = html_entity_decode($left_content['description']);
    ...
    public function exportXmlForProductConfiguration()
    {
        ...
        foreach ($products as $product) {
            ...
            foreach ($categories as $item) {
                ...
                $nameCategory = $this->convertNameUrl($category[1]);
                $idCategory = $category[0];

                $currentTrack = $domtree->createElement("url");
                $currentTrack = $xmlRoot->appendChild($currentTrack);
                $currentTrack->appendChild($domtree->createElement('loc', $url . '/duschrueckwand/' . $nameCategory . '/' . $product_name . '/' . $product_id . '/' . $idCategory . '/2'));
        ...
        echo 'Wrote: ' . $domtree->save("./sitemap_product.xml") . ' bytes';
    }
