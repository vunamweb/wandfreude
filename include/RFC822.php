...
© Copyright 2005 Chuck Hagenbuch
...
class Mail_RFC822
{
	...
	*/
	private function _validatePhrase($phrase)
	{
		...
		for ($i = 0; $i < count($phrase_parts); $i++) {
			...
			if ( !$this->_validateAtom($phrase_parts[$i],'phrase') ) {
				return false;
			}
		}
		return true;
	...
	* 3.1.4. STRUCTURED FIELD BODIES - atoms
	*
	* @access private
	* @param string $atom The string to check.
	* @return boolean Success or failure.
	*/
	private function _validateAtom($atom,$type='subdomain')
	{
		if (!$this->validate) {
	...
	*/
	private function _validateQuotedString($qstring)
	{
		...
		// Perform check.
		## adjust: this is weird syntax, very unreadable. FIX
	...
	*/
	private function _validateSubdomain($subdomain)
	{
		if (preg_match('|^\[(.*)]$|', $subdomain, $arr)){
			...
			if ( !$this->_validateAtom($subdomain,'subdomain') ) {
				return false;
			}
		}
