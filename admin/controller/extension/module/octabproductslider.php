	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('octabproductslider', $this->request->post);
			} else {
		...
		$data['text_type_bestseller'] = $this->language->get('text_type_bestseller');
		$data['text_type_mostviewed'] = $this->language->get('text_type_mostviewed');
		$data['text_type_random'] = $this->language->get('text_type_random');
		$data['text_type_special'] = $this->language->get('text_type_special');
		$data['text_type_latest'] = $this->language->get('text_type_latest');
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/octabproductslider', 'token=' . $this->session->data['token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/octabproductslider', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true)
			);			
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/octabproductslider', 'token=' . $this->session->data['token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/octabproductslider', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['types'])) {
			$data['types'] = $this->request->post['types'];
		} elseif (!empty($module_info)) {
			$data['types'] = $module_info['types'];
		} else {
			$data['types'] = array();
		}
		...
		$this->response->setOutput($this->load->view('extension/module/octabproductslider', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/octabproductslider')) {
