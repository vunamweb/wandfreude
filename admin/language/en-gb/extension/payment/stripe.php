...
$_['help_stripe_settings']				= 'API Keys can be found in your Stripe admin panel under Developers > API Keys';
...
$_['entry_three_d_secure']				= '3D Secure:';
...
$_['entry_charge_mode']					= 'Charge Mode: <div class="help-text">Choose whether to authorize payments and manually capture them later, or to capture (i.e. fully charge) payments when orders are placed. For payments that are only Authorized, you can Capture them by using the link provided in the History tab for the order.<br><br>If you choose "Authorize if possibly fraudulent, Capture otherwise" then the extension will use your OpenCart fraud extensions to determine whether an order might be fraudulent. If the fraud score is over your threshold, the charge will be Authorized; if under, the charge will be Captured.</div>';
...
<ul>
	...
	<li>If you have a coupon set up in your Stripe account, you can map an OpenCart coupon to it by using the same coupon code and discount amount. When a customer purchases a subscription product and uses that coupon code, it will pass the code to Stripe to properly adjust the subscription charge.</li><br>
...
$_['entry_order_address']				= 'Subscription Order Address: <div class="help-text">Choose whether to use the customer\'s default address in OpenCart or the customer\'s address in Stripe for the address on orders created when a subscription is charged. You can also choose to use the Stripe address as the billing address, and the customer\'s default OpenCart address as the shipping address.<br><br>A customer\'s billing and shipping address will be updated in Stripe whenever they use a stored card, store a new card, or purchase a subscription product. There\'s no easy way to stop that at this point, but if you want to disable that functionality contact Clear Thinking to discuss it.</div>';
$_['text_stripe_address']				= 'Use Stripe address';
$_['text_opencart_address']				= 'Use OpenCart address';
$_['text_stripe_and_opencart']			= 'Use Stripe for billing, and OpenCart for shipping';
