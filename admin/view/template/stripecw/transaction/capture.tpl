...
<div id="content">
    ...
    <div class="panel panel-default">
			...
			<?php if ($transaction->getTransactionObject()->isPartialCapturePossible()):?>
				<h2><?php echo StripeCw_Language::_('Partial Capture'); ?></h2>
				<p><?php echo StripeCw_Language::_('With the following form you can perform a partial capture.'); ?></p>
				<form action="<?php $captureConfirmUrl; ?>" method="POST" class="stripecw-line-item-grid " id="capture-form">
					...
					<table class="table table-bordered table-hover">
						...
						<tbody>
						<?php foreach ($transaction->getTransactionObject()->getUncapturedLineItems() as $index => $item):?>
							<?php 
						...
						<tfoot>
							<tr>
								<td colspan="6" style="text-align: right;"><?php echo StripeCw_Language::_('Total Capture Amount'); ?>:</td>
								<td id="line-item-total" style="text-align: right;">
								<?php echo Customweb_Util_Currency::formatAmount($transaction->getTransactionObject()->getCapturableAmount(), $transaction->getTransactionObject()->getCurrencyCode()); ?> 
								<?php echo strtoupper($transaction->getTransactionObject()->getCurrencyCode());?>
							</tr>
						</tfoot>
					</table>
					<?php if ($transaction->getTransactionObject()->isCaptureClosable()):?>
						<div class="closable-box">
							<label>
								<input id="close-transaction" type="checkbox" name="close" value="on" /> <?php echo StripeCw_Language::_('Close transaction for further captures'); ?>
							</label>
					...
					<div style="text-align: right;">
						<input type="submit" class="btn btn-success" value="<?php echo StripeCw_Language::_('Capture'); ?>" />
					</div>
				</form>
			<?php endif;?>
