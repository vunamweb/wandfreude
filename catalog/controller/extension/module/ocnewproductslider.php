...
class ControllerExtensionModuleOcnewproductslider extends Controller {
	public function index($setting) {
		...
		foreach ($results as $result) {
				if($result['product_id'] != $f_product['product_id']) {
					$productsExceptFirst[] = $result;
				}
			}
		
			foreach ($productsExceptFirst as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], $setting['width'], $setting['height']);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', $setting['width'], $setting['height']);
				...
				$data['products_excpt_first'][] = array(
					'product_id'  => $result['product_id'],
					'thumb'       => $image,
					'rotator_image' => $rotator_image,
					'name'        => $result['name'],
