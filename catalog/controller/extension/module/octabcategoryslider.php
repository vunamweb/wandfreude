...
class ControllerExtensionModuleOctabcategoryslider extends Controller {
	public function index($setting) {
		...
		foreach($arrayCates as $cate_id) {
			if(in_array($cate_id,$arrayExisted)) {
				$filter_data['filter_category_id'] = $cate_id;
				$f_product = $this->getFirstProduct($filter_data,$setting);
				$fproductByCates[$cate_id] = $f_product;
			}			
		}

		$productByCatesExceptFirst = array();
		foreach($arrayCates as $cate_id) {
			if(in_array($cate_id,$arrayExisted)) {
				$filter_data['filter_category_id'] = $cate_id;
				$results = $this->getProductFromDataExceptFirst($filter_data,$setting);
				$productByCatesExceptFirst[$cate_id] = $results;
			}			
		}
		...
		$data['category_products_excpt_first'] = $productByCatesExceptFirst;
		$data['category_fproduct'] = $fproductByCates;
...
public function getFirstProduct($data= array(), $setting = array()) {
		$products = $this->getProductFromData($data, $setting);
		$f_product = array();
		$count = 0;
	...
	public function getProductFromDataExceptFirst($data= array(), $setting = array()) {
		$products = $this->getProductFromData($data, $setting);
		$fproduct = $this->getFirstProduct($data, $setting);

		$productsExceptFirst = array();

		foreach ($products as $product) {
			if($product['product_id'] != $fproduct['product_id']) {
				$productsExceptFirst[] = $product;
			}
		}

		return $productsExceptFirst;
	}
