...
class ModelCatalogCategory extends Model
{
    ...
    {
        $sql = "select product_id from " . DB_PREFIX . "product_to_category where category_id = " . $category_id . " order by product_id desc";
        $query = $this->db->query($sql);
        $query = $query->rows;
    ...
    public function getCategoriesPlate($parent_id = 0)
    {
        ...
        foreach ($categories as $category) {
            $query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product_to_category pc, " . DB_PREFIX . "product p where p.product_id = pc.product_id and pc.category_id = " . $category[category_id] . " order by p.sort_order");
