<ul class="list_category d-flex justify-content-center">
        <?php foreach ($categories as $category) { ?>
            <?php if($category['count'] > 0){ ?>
               <li><a href="javascript:void(0)"  class="category-slider-homepage <?php if($currentCategory == $category[category_id] ) echo 'active' ?>" id="<?php echo $category[category_id] ?>"><?php echo $category[name] ?></a></li>
            <?php } ?>
        <?php } ?>
</ul>
...
<div class="random-products-slider">
  ...
  <?php if($products): ?>
    ...
    <div class="product-layout product-grid layout2 xxx">
							  <div class="product-thumb">
								  ...
								  <?php if($product['category_id'] != ''){
								      $href= 'duschrueckwand' . '/' . $product[category_name] . '/' .$product[name] . '/' . $product[product_id] . '/' . $product[category_id] . '/2' ;
								 ?>
                                     <a href="<?php echo $href ?>">
								  <?php if ($product['thumb']) { ?>
									<img src="mthumb.php?q=50&amp;w=300&amp;h=200&amp;zc=1&amp;src=image/<?php echo $product['url_image']; ?>" alt="<?php echo $product['name']; ?>" title="<?php echo $product['name']; ?>" class="img-responsive lazy" width="300" height="200" />
...
$(document).ready(function() {
  ...
  $('.category-slider-homepage').click(function(e){
       var id = $(this).attr('id');

       $.ajax({
		url: 'index.php?route=extension/module/ocrandomslider/load&category='+id+'',
		type: 'GET',
		beforeSend: function() {
