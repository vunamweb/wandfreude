...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info && ($product_info['type_product'] == 0 || isset($_GET['type_product']))) {
			...
			//$this->document->addLink('product/'.$this->request->get['product_id'].'/'.$this->request->get['category'].'/'.$this->request->get['number_plate'].'', '', '');
...
if(file_exists('catalog/model/extension/gaenhproprodpage.php')) { 
	...
	public function add(){
	   ...
	   $file = str_replace("index.php", "", $_SERVER['SCRIPT_FILENAME']) . 'image/uploads/'.$filename;  
	   //echo $file;
	...
	public function getPriceMaterial()
	{
		...
		if ($qm > 0.1) $round_qm = $this->floor_dec($qm, 1, '.');
		else $round_qm = $this->floor_dec($qm, 2, '.');
		// $round_qm = round($qm,1);
		$this->load->model('catalog/product');
	...
	public function floor_dec($number,$precision = 2,$separator = '.') {
		...
		if($numberpart[0]>=0) {
  		...
  		if($numberpart[1]==0) return $numberpart[0];
  		else {
	  		$ceil_number= array($numberpart[0],$numberpart[1]);
	  		return implode($separator,$ceil_number);
	  	}
